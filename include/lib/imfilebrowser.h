...
enum ImGuiFileBrowserFlags_
{
    ...
    ImGuiFileBrowserFlags_NoModal           = 1 << 2, // file browsing window_ is modal by default. specify this to use a popup window_
    ImGuiFileBrowserFlags_NoTitleBar        = 1 << 3, // hide window_ title bar
    ImGuiFileBrowserFlags_NoStatusBar       = 1 << 4, // hide status bar at the bottom of browsing window_
...
namespace ImGui
{
    ...
    public:
        ...
        // set the window_ size (in pixels)
        // default is (700, 450)
        void SetWindowSize(int width, int height) noexcept;

        // set the window_ title text
        void SetTitle(std::string title);

        // open the browsing window_
        void Open();

        // close the browsing window_
        void Close();

        // the browsing window_ is opened or not
        bool IsOpened() const noexcept;

        // display the browsing window_ if opened
        void Display();
...
inline void ImGui::FileBrowser::Display()
{
    ...
    // browse files in a child window_
